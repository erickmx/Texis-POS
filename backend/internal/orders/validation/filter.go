...
var AllowedOrderStatuses = []string{"pending"}
var AllowedOrderSortColumns = []string{"created_at", "folio", "total_amount"}
...
func ParseOrderFilter(values map[string]string) OrderFilter {
	f := OrderFilter{
		Page:      1,
		Limit:     20,
		SortBy:    "created_at",
...
func (f OrderFilter) Validate() []FieldError {
	var errs []FieldError
	if f.Page < 1 {
	...
	if f.Status != "" && !isInSlice(f.Status, AllowedOrderStatuses) {
		errs = append(errs, FieldError{Field: "status", Message: fmt.Sprintf("invalid status: %s. Allowed: %s", f.Status, strings.Join(AllowedOrderStatuses, ", "))})
	}
	if !isInSlice(f.SortBy, AllowedOrderSortColumns) {
		errs = append(errs, FieldError{Field: "sort_by", Message: fmt.Sprintf("invalid sort_by: %s. Allowed: %s", f.SortBy, strings.Join(AllowedOrderSortColumns, ", "))})
	}
